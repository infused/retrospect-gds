 ...
 */
	...
	define('DEFAULT_RESULT_ROWS', 100);
	define('MAX_RESULT_ROWS', 500);
	$cr = DEFAULT_RESULT_ROWS;
	...
	if (isset($_GET['start'])) {
		...
		$cr = 0;
		if (isset($_GET['count']))
			$cr = intval($_GET['count']);
		if ($cr == 0)
			$cr = DEFAULT_RESULT_ROWS;
		else if ($cr > MAX_RESULT_ROWS)
			$cr = MAX_RESULT_ROWS;
		$res_rows = $_SESSION['res_rows'];
	} else {
		...
		# name searches
		if ($search_type == 'name') {
			$sql = "SELECT indkey FROM " . TBL_INDIV . " WHERE ";
		...
		while ($row = $rs->FetchRow())
			$res_rows[] = $row['indkey'];
		$_SESSION['res_rows'] = $res_rows;
		$start = 1; # counting from one not zero
	}

	# some logic for the browse links
	$ci = count($res_rows);
	# "First" link
	if ($start >= $ci)
		$start = $ci - $cr + 1;
	...
	if ($prev_start < 1)
		...
		$str_prev = 'start=' . $prev_start . $str_c;
	...
	if ($next_start > $ci)
		...
		$str_next = 'start=' . $next_start . $str_c;

	# "Last" link
	$last_start = (intval(($ci - $start) / $cr)) * $cr + $start;
	$str_last = 'start=' . $last_start . $str_c;

	$individuals = array();
	for ($i = $start - 1; $i < $end; $i++) {
				$o = new Person($res_rows[$i], 1);
				keyword_push($o->full_name());
				$individuals[] = $o;
	}
	$smarty->assign('individuals', $individuals);
	$smarty->assign('individuals_count', $ci);
	$smarty->assign('start', $start);
	$smarty->assign('end', $end);
	$smarty->assign('first', $str_first);
	$smarty->assign('prev', $str_prev);
	$smarty->assign('next', $str_next);
	$smarty->assign('last', $str_last);
?>
