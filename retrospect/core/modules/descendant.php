 ...
 */
	...
	*/
	...
	$smarty->assign_by_ref('page_title', sprintf(gtc("Descendant Report for %s"), htmlentities($o->name)));
	...
	function display_indiv($p_array) {
		...
		foreach ($p_node->marriages as $marriage) {
			...
			if ($marriage->child_count > 0) {
				...
				foreach ($marriage->children as $child_indkey) {
					...
					$child_nk = '<a class="secondary" href="'.$_SERVER['PHP_SELF'].'?m=family&amp;id='.$child->indkey.'">'.htmlentities($child->name).'</a>';
		...
		$individual['name_link'] = '<a href="'.$_SERVER['PHP_SELF'].'?m=family&amp;id='.$p_node->indkey.'">'.htmlentities($p_node->name).'</a>';
