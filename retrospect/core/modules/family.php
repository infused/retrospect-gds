 ...
 */
	...
	$smarty->assign_by_ref('page_title', sprintf(gtc("Family Page for %s"), htmlentities($o->name)));
	...
	if ($o->father_indkey) { 
		...
		keyword_push(htmlentities($f->name));
		unset($f, $params);
	}
	...
	if ($o->mother_indkey) { 
		...
		keyword_push(htmlentities($m->name));
		unset($m, $params);
	}
	...
	for ($i = 0; $i < count($o->marriages); $i++) {
		...
		if ($s->name) { keyword_push(htmlentities($s->name)); }
		...
		$marriages[$i]['spouse_link'] = Theme::BuildLink($params, htmlentities($s->name));
		...
		if ($m->child_count > 0) {
			...
			foreach ($m->children as $child_indkey) {
				...
				keyword_push(htmlentities($c->name));
				$params = array('m'=>'family','id'=>$c->indkey);
				$children[] = array('child_link'=>Theme::BuildLink($params, htmlentities($c->name)), 'child'=>$c);
			}
